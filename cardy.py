...
class RfidObserver(CardObserver):
    def __init__(self, card_atrs, uid_repo, event_insert, event_remove, event_comm_error, event_first_card):
        ...
        self._atr_inserted = nfcplaylistconsts.NO_ATR
        self._id_inserted = nfcplaylistconsts.NO_CARD_ID
    ...
    def update(self, observable, actions):
        ...
        for card in removed_cards:
            ...
            if self._atr_inserted != nfcplaylistconsts.NO_ATR:
                id, ok = self.calc_card_id_removed(card)
                if ok:
                    pygame.event.post(pygame.event.Event(self._ev_remove, card_id=id))
                else:
                    pygame.event.post(pygame.event.Event(self._ev_comm_error, err_type=nfcplaylistconsts.ERR_TYPE_COMM, err_msg="Card error at removal"))
        ...
        for card in added_cards:
            ...
            if self._atr_inserted == nfcplaylistconsts.NO_ATR:
                ...
                if ok:
                    ...
                    pygame.event.post(pygame.event.Event(self._ev_comm_error, err_type=nfcplaylistconsts.ERR_TYPE_COMM, err_msg="Card error at insertion"))
